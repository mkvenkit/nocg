...
vec3 getTorusCol(vec3 p)
{
    vec3 col;

    if (sin(100*(p.x + p.y)) > 0.5) {
...
{
  // torus
  float d = sdfTorus(p, vec2(2, 0.5));
  Surf torus = Surf(d, getTorusCol(p));
  ...
  // return min 
  //return dist; 
  return minSurf(minSurf(minSurf(torus, cylX), cylY), cylZ);
}
